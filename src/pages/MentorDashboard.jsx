...
import {
  ...
  Check, Users, Send, PhoneCall, Trash2, Map, ChevronRight
} from 'lucide-react';
...
const TABS = [
  ...
  { id: 'requests',     label: 'Session Requests', icon: Inbox },
          ...
          <nav className="space-y-2">
            {TABS.map(tab => (
              <NavItem
                ...
                badge={tab.id === 'requests' ? pendingCount : tab.id === 'connections' ? connectionCount : 0}
              />
            ))}
          </nav>
        </div>
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
          ...
          <div className="flex items-center gap-4">
            <button className="p-3 bg-white border border-slate-200 rounded-2xl text-slate-400 hover:text-emerald-500 shadow-sm relative"
              onClick={() => setActiveTab(connectionCount > 0 ? 'connections' : 'requests')}
            >
              <Bell size={20} />
              {(pendingCount + connectionCount) > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 bg-red-500 text-white text-[9px] font-black rounded-full flex items-center justify-center">
                  {(pendingCount + connectionCount) > 99 ? '99+' : pendingCount + connectionCount}
                </span>
              )}
            </button>
